...
6. **SimHub plugin**: `cd apps/plugin/WinPodiums.Plugin && dotnet restore && dotnet build --configuration Release --no-restore`. Copy built DLL to `C:\Program Files (x86)\SimHub\` (SimHub install root; the only SimHub deploy path this repo supports); restart SimHub; point plugin at http://localhost:8787. Auth: browser (PKCE) primary; manual token debug-only via /auth/token.
