...
- [ ] **Before pushing to a remote branch:** Run local tests; **at least 80% of tests must pass** before pushing (API unit tests, optional smoke). See [Run tests before push](../../../docs/guides/development.md#run-tests-before-push). Block or warn on push if the threshold is not met.
