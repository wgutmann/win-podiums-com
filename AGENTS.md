...
- **SimHub**: Plugin work; use the simhub-plugin-builder skill when relevant. To build and deploy the plugin to SimHub, use the simhub-plugin-deploy skill (run `scripts/deploy-plugin.ps1`).
...
- **PR summary:** When generating or filling in a pull request (e.g. Summary section), **ALWAYS include** the PRD and tech plan links: stable IDs (PRD-XXX, TP-XXX) and markdown links to the docs (e.g. `[PRD-001](docs/product/…)`, `[TP-SPOC-001](docs/tech-plans/…)`). Fill the Traceability section with the same IDs so ContextStream can map PR ↔ Tech Plan ↔ PRD. See [.github/PULL_REQUEST_TEMPLATE.md](.github/PULL_REQUEST_TEMPLATE.md) and [ContextStream mapping §1.4](docs/guides/contextstream-mapping.md#14-linking-pull-requests-to-tech-plans-and-prds-graph-visible).
- **PR description and ContextStream (mandatory for SimHub / Phase 1 PRs):** When creating or filling a PR that implements **PRD-001** or **TP-SPOC-001–005** (SimHub plugin / Phase 1 work), the agent **MUST** automatically populate the PR description from [.github/PR_DESCRIPTION_SIMHUB_PLUGIN.md](.github/PR_DESCRIPTION_SIMHUB_PLUGIN.md): copy the **Summary**, **PRD and tech plan table**, and **Traceability** (including **Doc paths**). This applies on the first PR that touches this scope (i.e. when the PR is created for a branch implementing PRD-001 / TP-SPOC). After the PR is opened or merged, **capture an implementation event** in ContextStream (when MCP is available): `session(action="capture", event_type="implementation", title="PR #<number>: <short summary> (TP-SPOC-004)", content="<PR URL>. Implements TP-SPOC-001–005, PRD-001.", provenance={ pr_url: "<PR URL>" }, code_refs=[ { file_path: "docs/product/simhub-plugin-poc/001-simhub-plugin-poc.md" }, ... ])` using the Doc paths from the Traceability section. When opening the PR, add labels **prd-PRD-001** and **tech-plan-TP-SPOC-004** (and **tech-plan-TP-SPOC-001** through **TP-SPOC-005** as desired) for filtering. See [.cursor/rules/pr-contextstream.mdc](.cursor/rules/pr-contextstream.mdc) and [ContextStream mapping §1.4](docs/guides/contextstream-mapping.md#14-linking-pull-requests-to-tech-plans-and-prds-graph-visible).
