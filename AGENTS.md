...
- **Docker**: Dev environment; use the docker-dev-environment skill. Run and test locally with Docker; tests run against the Dockerized API so config stays 1:1. When **prepare env** includes the SimHub plugin: **actively stop SimHub** (Stop-Process -Force), **verify** it is closed (Get-Process; do not proceed until zero SimHub processes), then run the plugin copy. Do not move on from the SimHub step until verified closed (see docker-dev-environment skill §3c).
