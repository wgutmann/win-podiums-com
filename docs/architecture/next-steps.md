...
| **Configure Discord + secrets** | `apps/api` | Copy `.dev.vars.example` to `.dev.vars`; set `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `SESSION_SECRET`. In Discord Developer Portal, add redirect URIs: **`http://localhost:8787/auth/callback`** (web login); for plugin "Link to Discord" also add **`http://127.0.0.1:54321/callback`** (per [TP-SPOC-002](../tech-plans/simhub-plugin-poc/002-auth-pkce-token-storage.md)). Add production URL when you deploy. |
