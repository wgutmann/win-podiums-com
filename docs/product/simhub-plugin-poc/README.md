...
**ContextStream**: PRD-SPOC-001 and tech plans TP-SPOC-001–005 use stable IDs and **Related** / **Implements** for knowledge graph linking. See [ContextStream mapping](../guides/contextstream-mapping.md).

## Documents

| Document | Status | Version | Technical Plan | Description |
|----------|--------|---------|----------------|-------------|
| [PRD-SPOC-001: SimHub Plugin POC](001-simhub-plugin-poc.md) | Draft | 1.0 | [TP-SPOC-001](../tech-plans/simhub-plugin-poc/001-plugin-skeleton-sdk-config.md)–[005](../tech-plans/simhub-plugin-poc/005-poc-testing-completion.md) | Minimal plugin: browser auth (PKCE) primary; manual token debug-only, feature-flagged; one verification API call (heartbeat) |
...
Out of scope for the POC: full Scrutineering Panel UI, QR code auth in plugin (browser PKCE is the primary in-scope method), race submission, HMAC signing, full Telemetry Proof layers, installer/updater.
...
- [Tech plans (SimHub POC)](../../tech-plans/simhub-plugin-poc/README.md) — TP-SPOC-001–005 with Implements → PRD-SPOC-001
